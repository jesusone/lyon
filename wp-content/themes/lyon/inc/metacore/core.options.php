    ...
    if (is_admin() && !empty($params['id']) && isset($core_options)) {
        ...
        elseif ($pagenow=='post-new.php' || $pagenow=='post.php'){
            ...
            */
            ...
            $value = array($params['id']);
            // Get value
            if(isset($zo_meta->{$value[0]})){
                $params['value'] = $zo_meta->{$value[0]};
            } else {
                $params['value'] = null;
            }
...
class ZoCoreControl
{
    ...
    public function metabox($params){
        ob_start();
        ?>
		<div id="zo_metabox_field_<?php echo esc_attr($params['id']); ?>" class="zo_metabox_field clearfix">
